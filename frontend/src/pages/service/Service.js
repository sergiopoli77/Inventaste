...
const Service = () => {
  ...
  // Fetch categories to get Service category ID
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const serviceCategory = data.data.find(category => category.name === "Service");
          if (serviceCategory) {
  ...
  // Fetch service items once we have the service category ID
  useEffect(() => {
    const fetchServiceItems = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const serviceItems = data.data.filter(item => 
            item.id_kategori && item.id_kategori._id === serviceCategoryId
          );
  ...
  const handleEditItem = (id) => {
    const itemToEdit = serviceItems.find(item => item._id === id);
    if (itemToEdit) {
  ...
  const handleSaveItem = async (e) => {
    ...
    if (!serviceCategoryId) {
      alert("Service category not loaded yet");
      return;
    }
    ...
    if (window.confirm("Are you sure you want to delete this item?")) {
      try {
        ...
        if (response.ok) {
          ...
          setServiceItems(serviceItems.filter(item => item._id !== id));
        } else {
          const errorData = await response.json();
  ...
  const totalQuantity = serviceItems.reduce((sum, item) => sum + item.stock, 0);
  const outOfStockItems = serviceItems.filter(item => item.status === "non-aktif").length;
  const lowStockItems = serviceItems.filter(item => item.status === "low-stock").length;
